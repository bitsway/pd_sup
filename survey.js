...
var  apipath ='http://127.0.0.1:8000/pd_sup/'
...
/******** jahangirEditedEnd16 apipath *****************/
	...
	$('#rep_ul_list').empty();
	$('#rep_ul_list').append(localStorage.repListShow);
...
//		}
	...
	//$('#brand_list').empty();
	// $('#brand_list').append(localStorage.brnd_list);
		
		$('#product_ul_list').empty()
		$('#product_ul_list').append(localStorage.productListShow);
	
	//alert(localStorage.productListShow)
		
		//$('#req_ul_list').empty()
		//$('#req_ul_list').append(localStorage.repListShow);
...
function homePage() {
	...
	localStorage.today=today;		
			...
			var path_value1='http://w02.yeapps.com/pd/team_report_mobile/teamShow_web1?'
			var path_value2='http://w02.yeapps.com/pd/team_report_mobile/teamShow_web2?'
			var path_value3='http://w02.yeapps.com/pd/team_report_mobile/teamShow_web3?'
			
			var linkPath1="window.open('"+path_value1+"cid="+localStorage.cid+"&rep_id="+localStorage.user_id+"&rep_pass="+localStorage.user_pass	+"', '_system');"
			
			var linkPath2="window.open('"+path_value2+"cid="+localStorage.cid+"&rep_id="+localStorage.user_id+"&rep_pass="+localStorage.user_pass	+"', '_system');"
			
			var linkPath3="window.open('"+path_value3+"cid="+localStorage.cid+"&rep_id="+localStorage.user_id+"&rep_pass="+localStorage.user_pass	+"', '_system');"
			...
			var button_combo_show='<table width="100%" border="0"  style="background-color:#09C"><tr>'
			...
			$('#button_show_div').empty();
			$('#button_show_div').append(button_combo_show);
			...
			$('#drPage').empty();
			$('#drPage').append(dr_button_combo_show);
					
	//localStorage.synced=synced
	//if ((localStorage.synced=='YES') & (localStorage.sync_date==today)){
	//if (localStorage.synced!='YES'){
		//alert('2')
		//if (localStorage.user_type=='sup'){
...
function getLocationInfoSubmit() { //location
	//$("#lat").val(0);
//	$("#longitude").val(0);
//	
//	
...
function afterSync(){
		localStorage.prouct_stringSubmit=''
		localStorage.show_data=''
		$("#show_data").html('');
...
//alert('ds')
	...
	cid=$.trim(cid);
    //var apipath_base_photo_dm ='http://127.0.0.1:8000/pd_sup/syncmobile_417_new/dmpath?CID='+cid +'&HTTPPASS=e99business321cba'
	var apipath_base_photo_dm ='http://w02.yeapps.com/pd/syncmobile_417_new_sup/dmpath?CID='+cid +'&HTTPPASS=e99business321cba'
								...
								success:function(data, status,xhr){	
										...
										if (resultArray[0]=='FAILED'){
											...
											$("#error_login").html(resultArray[1]);
													...
													localStorage.reptStr=resultArray[11]
													
													localStorage.supStr=resultArray[12]
													localStorage.reqrepStr=resultArray[13]
													
													localStorage.suprepStr=resultArray[14]
													
													var repListShow=''
													var rep_i_show=''

													localStorage.repListShow=''
													var sup_rep_list=localStorage.suprepStr.split('<rdrd>');
													
													var sup_rep_listLength=sup_rep_list.length
													for(var s=0; s < sup_rep_listLength-1; s++){
														
														var inpRepId=s.toString()
														var repArray=sup_rep_list[s].split('<rdrd>')
														var repId=repArray[0]
														
					repListShow=repListShow+'<li class="name" style="font-weight:bold" onClick="rep_reqApprv_list(\''+repId+'\')">'+repId+'</li>'
													
														//rep_i_show=rep_i_show+'<input  name="'+inpRepId+'" id="'+inpRepId+'" value="'+repId+'" type="hidden">'
													...
													//alert (repListShow)
													localStorage.repListShow=repListShow
													localStorage.rep_i_show=rep_i_show
													...
													var companyListListShowLength=companyList.length	
													
													
													for (var k=0; k < companyListListShowLength-1; k++){
														var input_id=k.toString()
													
														var companyValueArray1 = companyList[k].split('<fdfd>');
													...
													//$('#rep_ul_list').empty()
													//$('#rep_ul_list').append(localStorage.companyListShow);
													localStorage.company_i_Show=company_i_Show
													...
													var productList = localStorage.productListStr.split('<ITEMEND>');
//												
													var productListShowLength=productList.length
													for (var i=0; i < productListShowLength-1; i++){
														if (i==0){var pr_A=productList[i].replace('<ITEMSTART>','');}
														if (i==1){var pr_B=productList[i].replace('<ITEMSTART>','');}
														if (i==2){var pr_C=productList[i].replace('<ITEMSTART>','');}
...
function reppageList(){
	
	$("#req_ul_list").val();
	var company_combo_list=localStorage.companyListShow;
	
	$('#req_ul_list').empty();
	$('#req_ul_list').append(company_combo_list);
	$.afui.loadContent("#requisition_page",true,true,'right');	
}
...
function repapvList(){
	//alert('sda')
	$("#rep_ul_list").val();
	var rep_combo_list=localStorage.repListShow;
	
	$('#rep_ul_list').empty();
	$('#rep_ul_list').append(rep_combo_list);
	$.afui.loadContent("#rep_list_page",true,true,'right');	
}
...
function rep_reqApprv_list(repId){
	//alert (repId)
	...
	timeout: 30000,
	error: function(xhr) {
	$("#reqsubmitError").html("Network error has occurred please try again!");
			$("#req_wait_image_login").hide();
	},
	success:function(data, status,xhr){	
		...
		if (resultArray[0]=='FAILED'){						
			...
			$("#req_wait_image_login").hide();
				...
				afterSync()
				localStorage.suprepStr=resultArray[1]

rep_req_show=''
				var rep_rep_list=localStorage.suprepStr.split('<recSep>');

//alert(resultArray[1])
		var rep_rep_listLength=rep_rep_list.length
					
					var sl_new=''
				rep_req_show=rep_req_show+'<table style="width:100%"><tr><td><b>Name</b></td><td><b>Id</b></td><td><b>Price</b></td><td><b>Qty</b></td><td><b>Ctn</b></td><td><b>CtnInfo </b></td></tr><tr>&nbsp;&nbsp;</tr>'	
				for(var s=0; s < rep_rep_listLength; s++){
							
							var sl_singleStr=rep_rep_list[s]
							
							
							var sl_single=sl_singleStr.split('<rdrd>')
							
							for(var i=0; i < sl_single.length-1; i++){
								var rowSingle=sl_single[i].split('<fdfd>')
								var slRow=rowSingle[0]
								var item_name=rowSingle[1]
								var item_id=rowSingle[2]
								var item_price=rowSingle[3]
								var item_piece=rowSingle[4]
								var item_carton=rowSingle[5]
								var carton_info=rowSingle[6]
								var Rstatus=rowSingle[7]
								var s_id='sl_'+slRow
								
								rep_req_show=rep_req_show+'<tr><td >'+item_name+'</td><td style="width:10%;">'+item_id+'</td><td style="width:10%;">'+item_price+'</td><td style="width:10%;">'+item_piece+'</td><td style="width:10%;">'+item_carton+'</td><td style="width:10%;">'+carton_info+'</td></tr>'
							}
		...
		//alert(item_sl)
								//var hi_f =''
						//if ( (sl_new.search(item_sl))!= -1 ){	
						//if(sl_new==''){sl_new=item_sl;
						//var hi_f ='<input type="hidden" id="sl_'+item_sl+'">'				
						//}
					//	if  ((item_sl==sl_new)&&(s_id.search(rep_req_show))== -1){
						
						if  ((sl_new!=slRow) & (sl_new!='') & ((Rstatus=='Submitted') | (Rstatus==''))){
							
							rep_req_show=rep_req_show+'<tr><td colspan="6"><table style="width:100%; background-color:#FFE6E6"><tr><td  style="width:50%"><input type="hidden" id="sl_'+slRow+'"><img id="wait_image_login"  src="ok.png" alt="" onClick="approveReq('+slRow+')"></td><td  style="width:50%"><img id="wait_image_login"  src="cancel.png" alt="" onClick="cancelReq('+slRow+')"></td></tr></table><br><br></td></tr>'
//							
						}
						else if  ((sl_new!=slRow) & (sl_new!='')  & ((Rstatus!='Submitted') | (Rstatus!=''))){
							
							rep_req_show=rep_req_show+'<tr><td colspan="6"><table style="width:100%; background-color:#FFE6E6"><tr><td  style="width:50%"><input type="hidden" id="sl_'+slRow+'"></td><td  style="width:50%">'+Rstatus+'</td></tr></table><br><br></td></tr>'
//							
						}
						sl_new=slRow						
					
				}
				rep_req_show=rep_req_show+'</table>'
				localStorage.rep_req_show=rep_req_show
				
				$('#repreq_list').empty();
				$('#repreq_list').append(localStorage.rep_req_show);
	...
	// reqrep_list_uladdMarketList()
}


function rep_reqApprv_listShow(repId){
	$("#req_wait_image_login").show();
	localStorage.repReq=repId
	//alert (localStorage.base_url+'reqList?cid='+localStorage.cid+'&rep_id='+localStorage.user_id+'&rep_pass='+localStorage.user_pass+'&synccode='+localStorage.synccode+'&repId='+repId)
	$.ajax(localStorage.base_url+'reqList?cid='+localStorage.cid+'&rep_id='+localStorage.user_id+'&rep_pass='+localStorage.user_pass+'&synccode='+localStorage.synccode+'&repId='+repId,{
...
function approveReq(sl){
	
	$("#req_wait_image_login").show();
	$.ajax(localStorage.base_url+'approvReq?cid='+localStorage.cid+'&rep_id='+localStorage.user_id+'&rep_pass='+localStorage.user_pass+'&synccode='+localStorage.synccode+'&reqsl='+sl,{
	// cid:localStorage.cid,rep_id:localStorage.user_id,rep_pass:localStorage.user_pass,synccode:localStorage.synccode,
	type: 'POST',
	timeout: 30000,
	error: function(xhr) {
	...
	success:function(data, status,xhr){	
		...
		if (resultArray[0]=='FAILED'){						
			$("#reqsubmitError").html('Network Timeout. Please check your Internet connection.');
			$("#req_wait_image_login").hide();
			//$("#visit_submit").show();
			//$("#visit_save_div").show();	
		}else if (resultArray[0]=='SUCCESS'){
				...
				rep_reqApprv_listShow(localStorage.repReq)
						$("#req_wait_image_login").hide();							
		}
		else{						
			$("#reqsubmitError").html('Network Timeout. Please check your Internet connection.');
...
function cancelReq(sl){
	$("#req_wait_image_login").show();
	$.ajax(localStorage.base_url+'cancelReq?cid='+localStorage.cid+'&rep_id='+localStorage.user_id+'&rep_pass='+localStorage.user_pass+'&synccode='+localStorage.synccode+'&reqsl='+sl,{
	// cid:localStorage.cid,rep_id:localStorage.user_id,rep_pass:localStorage.user_pass,synccode:localStorage.synccode,
	type: 'POST',
	timeout: 30000,
	error: function(xhr) {
...
function reqrep_list_uladdMarketList() {


	// var inputrep_id=s.toString()
	// alert(inputrep_id)
	
	// localStorage.rep_index=inputrep_id
	
	// var repList = localStorage.suprepStr.split('<rdrd>')
	// alert(repList)
	// var repstr=repList[inputrep_id]

	var rep_req_show=''
		var rep_rep_list=localStorage.suprepStr.split('<recSep>');
		
		var rep_rep_listLength=rep_rep_list.length
		for(var s=0; s < rep_rep_listLength-1; s++){
			
			var repreqArray=rep_rep_list[s].split('<rdrd>')
			 
			// var repId = repreqArray[1];
			
			rep_req_show=rep_req_show+'<li class="name"><table style="width:100%"><tr><td style="width:300px;">'+repreqArray+'</td><td style="width:300px;" ><img id="wait_image_login" style="" width="30px;" src="ok.png" alt="">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<img id="wait_image_login" style="" width="30px" src="cancel.png" alt=""></td></tr></table></li>'
			//alert(rep_req_show)
		}

		$("#repreq_list").val();
		var rep_combo_list=localStorage.rep_req_show;
		//alert(rep_combo_list)
		$('#repreq_list').empty();
		$('#repreq_list').append(rep_combo_list);
		
		//  $.afui.loadContent("#req_page_market",true,true,'right');	
...
function companyWsproduct(k){
	...
	for (var b=0; b < br_brnStrList.length-1; b++){
		...
		//var brnd_list=$('#brand_list').val()
		//alert(brnd_list)
		//localStorage.brnd_list=brnd_list				
	product_list()	
...
function product_list(){
	...
	$.afui.loadContent("#product_page",true,true,'right');	
}


//=============================

function getCartCancelAfter(){
	//alert ('Nadira')
//localStorage.brnd_list=''
	...
	if (comp_index==1){proListStr=localStorage.pr_B}
	if (comp_index==2){proListStr=localStorage.pr_C}
...
localStorage.brnd_list=''
	...
	rep_StrM=localStorage.reptStr
	var rep_val='<option value="SelectPD">Select PD</option>'
	rep_Str=rep_StrM.split('<rd>')
	$('#rep_list').empty();
	//alert($('#rep_list').val())
	rep=''
	
	for (var b=0; b < rep_Str.length; b++){
			//rep_id = rep_Str[b]
			rep_name = rep_Str[b]
			//alert(rep_name)
			 rep_val=rep_val+'<option value="'+rep_name+'">'+rep_name+'</option>'

			//var rep_val='<option value="'+rep_name+'">'+rep_name+'</option>'
			//
			//$('#rep_list').append(rep_val);
			
		}
		 $('#rep_list').append(rep_val);
		var rep_list=$('#rep_list').val()
		//alert(rep_list)
		localStorage.rep_list=rep_list
...
function submit_data(){
	...
	//localStorage.supStr=supStr
	//alert(localStorage.supStr)
	//alert(localStorage.base_url+'dataSubmit?cid='+localStorage.cid+'&rep_id='+localStorage.user_id+'&rep_name='+localStorage.rep_name+'&password='+localStorage.user_pass+'&synccode='+localStorage.synccode+'&comp_Id='+localStorage.company_id+'&comp_name='+localStorage.company_name+'&prouct_string='+localStorage.prouct_stringSubmit+'&latitude='+localStorage.latitude+'&longitude='+localStorage.longitude+'&rep_list='+localStorage.filterG+'&supList='+localStorage.supStr+'&date='+localStorage.date)
	//$("#submitError").val(localStorage.base_url+'dataSubmit?cid='+localStorage.cid+'&rep_id='+localStorage.user_id+'&rep_name='+localStorage.rep_name+'&password='+localStorage.user_pass+'&synccode='+localStorage.synccode+'&comp_Id='+localStorage.company_id+'&comp_name='+localStorage.company_name+'&prouct_string='+localStorage.prouct_stringSubmit+'&latitude='+localStorage.latitude+'&longitude='+localStorage.longitude+'&rep_list='+localStorage.rep_list+'&supList='+localStorage.supStr+'&date='+localStorage.date)
	$.ajax(localStorage.base_url+'dataSubmit?cid='+localStorage.cid+'&rep_id='+localStorage.user_id+'&rep_name='+localStorage.rep_name+'&password='+localStorage.user_pass+'&synccode='+localStorage.synccode+'&comp_Id='+localStorage.company_id+'&comp_name='+localStorage.company_name+'&prouct_string='+localStorage.prouct_stringSubmit+'&latitude='+localStorage.latitude+'&longitude='+localStorage.longitude+'&rep_list='+localStorage.filterG+'&supList='+localStorage.supStr+'&date='+localStorage.date,{
			type:'POST',
			timeout: 30000,
			 error: function(result) {
						
						$("#submitError").html("Please Select PD ");
						$("#wait_image_login").hide();
						$("#loginButton").show();
						$("#SaveButton").show();
...
function marketNext_doc() {
	...
	var market_Id=marketNameId[1];
	       // alert (localStorage.base_url+'pharmacy_list?cid='+localStorage.cid+'&rep_id='+localStorage.user_id+'&rep_pass='+localStorage.user_pass+'&synccode='+localStorage.synccode+'&market_Id='+localStorage.market_Id)
			
	$.ajax({
				 type: 'POST',
				 timeout: 30000,
				 url: localStorage.base_url+'pharmacy_list?cid='+localStorage.cid+'&rep_id='+localStorage.user_id+'&rep_pass='+localStorage.user_pass+'&synccode='+localStorage.synccode+'&phrmcList='+localStorage.phrmcList,
				 
				 success: function(pharmacyStr) {
					if (pharmacyStr==''){		
							...
							var resultArray = pharmacyStr.split('<fdfd>');	
									//alert(resultArray)
								var pharmacyListShow=''
							
						
							for (var i=0; i < resultArray.length-1; i++){
								var pharmcValueArray = resultArray[i]
								//alert(pharmcValueArray)
								
							pharmacyListShow=pharmacyListShow+'<li style="" ><table style="width:100%"><tr><td style="width:10%;font-weight:bold;font-size:15px;">'+pharmcValueArray+'</td></tr></table></li>'
								//alert(screensettingsdataShow)									
							}
							 localStorage.pharmacyListShow=pharmacyListShow
							var pharmacylistAppend=$('#pharmacy_list');
							pharmacylistAppend.empty()
							pharmacylistAppend.append(pharmacyListShow);
							$.afui.loadContent("#pharmacy_page",true,true,'right');	
						}
				  },
...
function searchrep() {
	localStorage.filterG=''
	var filterG  = $("#rep_list").val().toUpperCase();
	localStorage.filterG=filterG
	//alert(filterG)
	var filter='|'+filterG
...
function uploadPhoto_docVisit(imageURI, imageName) {
	...
	ft.upload(imageURI, encodeURI(apipath_photo),winProfile,failProfile,options);
